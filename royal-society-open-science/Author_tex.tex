...
\corres{Loo, Tung Lun, Mohamad Khairi Ishak\\
\email{joshlootunglun@gmail.com, khairiishak@usm.my}}
...
In a typical workflow, a 3rd party code provider will need to sign their components with a private key and publish the public key. Then the OEM or users can enroll the public key to the database, which is stored in a UEFI authenticated variable region \cite{R5:13}.  During firmware boot process, the image verification procedure will verify the 3rd party code according to the image security database, using a verification flow entailed in Figure.
...
demonstrated in Figure 10. With this approach, the benefits of hardware-based security
scheme against software-based are highlighted
